...
NOTE: This section is only relevant if you're using a Raspberry Pi device older than Raspberry Pi 4, Compute Module 4, or Raspberry Pi 400.

If you're connecting from a non-RealVNC Viewer app, you need to downgrade VNC Server's authentication scheme, specify a password unique to VNC Server, and then use that password to authenticate.

If you're using a desktop environment, open the VNC Server on your Raspberry Pi, select menu:Menu[Options > Security], and choose *VNC password* from the *Authentication* dropdown.

If you're configuring a headless device, follow these steps:
 1. Open the config file in `/root/.vnc/config.d/vncserver-x11`.
 2. Replace `Authentication=SystemAuth` with `Authentication=VncAuth` and save the file.
 3. From the command line, run `sudo vncpasswd -service`. This will prompt you to set a password, and will insert that password in the config file for VNC Server running in Service Mode.
